...
- Additional notes for particularly important sections:
  - **iRMC User Registration `fsas.primergy.irmc_account_admin`**:
  ...
  - **SSL Certificate and CA Certificate Configuration `fsas.primergy.irmc_set_certificate`**:
  ...
  - **iRMC License Registration `fsas.primergy.irmc_set_license`**:
    Since licenses are issued per device,
    the `license_keys` parameter in `fsas.primergy.irmc_set_license` must be specified
  ...
  - **DNS Configuration for Windows Server `fsas.primergy.win_dns`**:
  ...
  - **Domain Joining `fsas.primergy.win_set_membership`**:
    When joining a domain, specify the DNS server that is associated with the domain controller
    as the first DNS server in the Windows server DNS settings `fsas.primergy.win_dns`.
  - **Data Drive Configuration `fsas.primergy.win_data_drive`**:
