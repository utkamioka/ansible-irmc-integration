...
- 特に注意が必要な箇所について補足します：
  - **iRMCのユーザ登録 `fsas_temp_ns.primergy.irmc_account_admin`**：
    権限（`role`=`Administrator`や`ipmi.lan_privilege`=`OEM`）を不用意に変更すると、
    それ以降の設定タスクがエラーになる可能性がありますのでご注意ください。
  - **SSL証明書・CA証明書設定 `fsas_temp_ns.primergy.irmc_set_certificate`**：
  ...
  - **ライセンス登録 `fsas_temp_ns.primergy.irmc_set_license`**：
  ...
  - **WindowsサーバーのDNS設定 `fsas_temp_ns.primergy.win_dns`**：
  ...
  - **ドメイン参加 `fsas_temp_ns.primergy.win_set_membership`**：
    ドメインに参加する場合は、WindowsサーバーのDNS設定`fsas_temp_ns.primergy.win_dns`で、
    １つ目のDNSサーバーにドメインコントローラーと連携しているDNSサーバーを指定してください。
  - **データドライブ設定 `fsas_temp_ns.primergy.win_data_drive`**：
